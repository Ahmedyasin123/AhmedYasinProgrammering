...
{ 	
	...
	String invertedWord= word;											//deklarerar det rätta ordet 
	...
	while(true) { 												//while-loop kollar om kraven under är uppfyllda
		...
		for (int i = 0; i < invertedWord.length(); i++){ 		
			if (invertedWord.charAt(i) == '-') {
				System.out.print(word.charAt(i));				// det kollas om ordet är lika med strecken och om den gör det skriver den ut det
			} else { 											
				// annars blir win false 
				win = false;
				System.out.print('-');							// annars blir win false 
